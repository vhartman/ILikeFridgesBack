...
def update_items(current_items, receipt_input):
    ...
    for product in receipt_input:
        ...
        flag = False
        if current_items:
            for curr_product in current_items:
                print curr_product['description']
                if curr_product['description'] == product:
                    # current_items += receipt_input[product]
                    changed_items.append(curr_product['id']-1)
                    flag = True
                    curr_product['amount'] += receipt_input[product]

            if flag == False:
                changed_items.append(len(current_items))
                current_items.append({'amount': receipt_input[product], 'description': product, 'id': len(current_items)+1}) #receipt_input[product]

        else:
            current_items = []
            changed_items.append(len(current_items))
            current_items.append({'amount': receipt_input[product], 'description': product, 'id': len(current_items)+1})
    return changed_items,current_items
